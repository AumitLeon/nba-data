...
def write_to_gcs(project: str, bucket_name: str, blob_name: str, data: dict):
    """Write a blob to GCS using file-like IO"""
...
def call_natstat_api(client: httpx.Client, url: str):
    ...
    # Use an aggressive timeout, since asking for more data per request leads to higher throughput at the cost of higher latency.
    r = client.get(url, timeout=60.0)
    return r


def call_natstat_api_with_pagination(
    client: httpx.Client, url: str, logger: logging.Logger
) -> dict:
    """
    Call the natstat API, write the data to GCS, and return the response.
    """
    ...
    logger.info(
        f"{r.status_code} response for page {current_page}/{total_pages} took: {time.time() - response_time_start} seconds"
    )

    padded_current_page = str(current_page).zfill(5)
    padded_total_pages = str(total_pages).zfill(5)
    file_name = (
        f"playerperfs/{date.today()}-{padded_current_page}-of-{padded_total_pages}.json"
    )
...
def main():
    ...
    while current_page != total_pages and next_page_url is not None:
        with httpx.Client(params=PARAMS) as client:
            response = call_natstat_api_with_pagination(
                client=client, url=next_page_url, logger=logger
            )
